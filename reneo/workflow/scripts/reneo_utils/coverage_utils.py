...
def add_component_external_endpoint_support(
...
def add_component_external_endpoint_pair_support_from_fields(
    endpoint_counts,
    read_name,
    read_orientation,
    mate_name,
...
def add_component_external_endpoint_spanning_read_support(
    endpoint_counts, left_read, right_read, target_contigs, contig_component_ids
):
    left_orientation = get_read_orientation(left_read)
    right_orientation = get_read_orientation(right_read)
...
def add_component_external_endpoint_sa_tag_support(
    endpoint_counts, read, target_contigs, contig_component_ids
):
    if not read.has_tag("SA"):
        return
...
def find_component_external_endpoint_read_support_in_bam(
    bam_queue, result_queue, target_contigs, contig_component_ids
):
    endpoint_counts = defaultdict(int)
    target_contigs = set(target_contigs)
        ...
        with pysam.AlignmentFile(bam_file, "rb") as bam:
            ...
            for contig in sorted(target_contigs.intersection(references)):
                for read in bam.fetch(contig):
                    ...
                    add_component_external_endpoint_sa_tag_support(
                        endpoint_counts, read, target_contigs, contig_component_ids
                    )
                    ...
                    if (
                        ...
                        mate_orientation = "-" if read.mate_is_reverse else "+"
                        add_component_external_endpoint_pair_support_from_fields(
                            endpoint_counts,
                            read.reference_name,
                            get_read_orientation(read),
                            read.next_reference_name,
            ...
            for alignments in reads.values():
                ...
                for i in range(len(alignments) - 1):
                    ...
                    seen_pairs.add(read_pair)
                    add_component_external_endpoint_spanning_read_support(
                        endpoint_counts,
                        left,
                        right,
                        target_contigs,
...
def get_component_external_endpoint_read_support(
    bam_path, output, target_contigs, contig_component_ids, nthreads
):
    """
    Count endpoint support from selected contig ends to contigs outside their component.
    """

    target_contigs = sorted(set(target_contigs))
    contig_component_ids = dict(sorted(contig_component_ids.items()))
    cache_file = f"{output}/component_external_endpoint_read_support.pickle"
    ...
    if os.path.isfile(cache_file):
        ...
        if (
            isinstance(cached, dict)
            and cached.get("target_contigs") == target_contigs
            and cached.get("contig_component_ids") == contig_component_ids
            and "counts" in cached
        ):
            return cached["counts"]
    ...
    for _ in range(nthreads):
        bam_queue.put(None)
        thread = threading.Thread(
            target=find_component_external_endpoint_read_support_in_bam,
            args=(bam_queue, result_queue, target_contigs, contig_component_ids),
        )
        threads.append(thread)
        thread.start()
        ...
        pickle.dump(
            {
                ...
                "contig_component_ids": contig_component_ids,
